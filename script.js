...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const TAGS = [
        'implementation', 'dp', 'math', 'graphs', 'data structures', 'greedy', 'strings', 
        'binary search', 'brute force', 'two pointers', 'sortings', 'bitmasks', 'trees', 
        'constructive algorithms', 'number theory', 'geometry', 'combinatorics', 'dsu'
    ];
    ...
    function renderRatingNav() {
        ...
        ratingNav.innerHTML = ratings.map(r => `<button class="rating-btn" data-rating="${r}">${r}</button>`).join('');
    }
    ...
    function renderProblemTable(problems) {
        ...
        if (problems.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="5" class="placeholder">No problems match your criteria.</td></tr>';
            return;
        }
    ...
    function setupEventListeners() {
        ratingNav.addEventListener('click', e => {
            if (e.target.classList.contains('rating-btn')) {
                ...
                state.selectedRating = state.selectedRating === newRating ? null : newRating;
    ...
    function modalActivities() {
        ...
        contactForm.addEventListener('submit', async function(event) {
            ...
            try {
                ...
                if (response.ok) {
                    contactFormStatus.textContent = "Thanks for your message! We'll get back to you soon.";
                ...
                contactFormStatus.textContent = error.message || 'An unknown error occurred.';
